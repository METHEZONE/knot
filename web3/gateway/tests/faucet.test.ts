...
const devnetConfig = loadConfig({ SOLANA_RPC_URL: "https://api.devnet.solana.com" });
...
test("loopback detection accepts local validators only", () => {
  ...
  assert.equal(isLoopbackRpc("https://api.devnet.solana.com"), false);
...
test("faucet refuses to run against a non-local cluster", async () => {
  const result = await airdrop(devnetConfig, { address: "11111111111111111111111111111111" });
