...
deploy_service "knot-web3" "${WEB3_IMAGE}" \
  --allow-unauthenticated \
  --set-env-vars="KNOT_SERVICE_NAME=knot-web3,GIT_SHA=${TAG},SOLANA_CLUSTER=devnet,SOLANA_RPC_URL=${SOLANA_RPC_URL:-https://api.devnet.solana.com},KNOT_WEB3_SIGNING_MODE=${KNOT_WEB3_SIGNING_MODE:-devnet},KNOT_ESCROW_PROGRAM_ID=${KNOT_ESCROW_PROGRAM_ID:-Aj63B5hLtvJdNQiAi61rMrgfW3pt8Lak3GQB59B6jysj},KNOT_USDC_MINT=${KNOT_USDC_MINT:-4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU},KNOT_SETTLEMENT_AUTHORITY=${KNOT_SETTLEMENT_AUTHORITY}" \
