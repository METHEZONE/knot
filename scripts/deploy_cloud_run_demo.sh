...
KNOT_ESCROW_NETWORK="${KNOT_ESCROW_NETWORK:-solanaDevnet}"
KNOT_USDC_MINT="${KNOT_USDC_MINT:-}"
KNOT_ESCROW_PROGRAM_ID="${KNOT_ESCROW_PROGRAM_ID:-}"
...
require_env "KNOT_USDC_MINT"
require_env "KNOT_ESCROW_PROGRAM_ID"

if [[ "${WEB3_SIGNING_MODE}" != "simulated" ]]; then
  echo "▸ Web3 Gateway will run in live ${SOLANA_CLUSTER} signing mode."
...
deploy_service "knot-web3" "${WEB3_IMAGE}" \
  ...
  --set-env-vars="KNOT_SERVICE_NAME=knot-web3,GIT_SHA=${TAG},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GCP_PROJECT_ID=${PROJECT_ID},SOLANA_CLUSTER=${SOLANA_CLUSTER},SOLANA_RPC_URL=${SOLANA_RPC_URL},KNOT_WEB3_SIGNING_MODE=${WEB3_SIGNING_MODE},KNOT_WEB3_AUTO_MINT_ON_LOCK=0,KNOT_WEB3_AUTO_SOL_TOPUP_ON_LOCK=0,KNOT_ESCROW_PROGRAM_ID=${KNOT_ESCROW_PROGRAM_ID},KNOT_USDC_MINT=${KNOT_USDC_MINT}" \
