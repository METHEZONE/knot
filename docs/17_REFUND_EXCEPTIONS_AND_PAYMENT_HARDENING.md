...
Updated: 2026-08-18
Owner: 효창 (블록체인 백엔드)
Status: 결정 반영 — 방향 확정(§0.5), N1·N3 확정(§0.6). **미정은 N2·N4 둘뿐**
...
### 가장 큰 노출 — 플랫폼이 유저 개인키를 보관한다

`walletCustody: "PLATFORM"` 은 플랫폼이 유저 키를 Secret Manager 에 보관하는 구조다(`libs/web3/user_wallet.py`). 이는 **타인을 위한 가상자산 보관·관리업**으로 해석될 소지가 크고, 그렇다면 특금법상 **가상자산사업자(VASP) 신고 의무**가 발생한다.

VASP 신고 요건:

| 요건 | 현실성 |
|---|---|
| ISMS 인증 | 비용·기간 부담이 크지만 가능 |
| **실명확인 입출금계정** (은행 발급) | **스타트업이 사실상 받기 어렵다** |
| AML/CFT 체계, 트래블룰 | 구축 가능 |

즉 **현재 임베디드 지갑 구조는 실서비스에서 그대로 쓸 수 없다.** 그리고 이것은 "web3 몰라도 쓸 수 있다" 는 제품 전제와 정면으로 충돌한다.

### 노출을 줄이는 구조 선택

| 선택 | 내용 | 평가 |
|---|---|---|
| 완전 비수탁 | Phantom 등 외부 지갑 필수 | 규제 안전. **제품 전제 포기** |
| 스마트 컨트랙트 지갑 / MPC | 플랫폼이 **단독으로 이전할 수 없는** 구조(2-of-2 등) | 커스터디 해석 회피 여지. 해석 리스크는 남음. 유력 |
| 자금이 플랫폼을 경유하지 않게 | 에스크로 → 크리에이터 **직접** 전송 | **이미 그렇게 되어 있다.** 반드시 유지 |

세 번째가 중요하다. `release_milestone` 은 vault 에서 `creator_destination` 으로 직접 보낸다. **플랫폼 지갑을 중간 경유지로 만드는 리팩터를 절대 하지 말 것** — 그 순간 플랫폼이 자금 이전을 업으로 하는 구조가 된다. (D5 수수료는 플랫폼 자기 수익이므로 성격이 다르다.)
...
| 5 | 커스터디 전환 경로 설계 (`PLATFORM` → 스마트월렛/MPC) | 유저가 쌓인 뒤 키 구조를 바꾸는 건 마이그레이션 지옥이다 |

1~4 는 지금 넣어도 비용이 거의 없다. **5 가 진짜 결정이고, 실서비스 목표 시점에서 역산해 판단해야 한다.**
...
| 8 | **P8 페널티** — 평판 위반 카운터 + 매칭 반영 | 3, 4 | `.agent/execplans/penalty-and-reputation.md` |
| 9 | P5 / P6 / P9-5 커스터디 전환 | 법무·파트너 | — |
...
방향은 §0.5, N1·N3 은 §0.6 에서 확정됐다. 남은 것은 N2·N4 둘이고, **둘 다 P0 프로그램 변경(재배포)을 막는다.** 전부 **협상 가능 항목의 기본값**이므로, 정하고 나서도 에이전트가 계약별로 조정할 수 있다(D1).

| # | 결정 | 제안 | 왜 정해야 하나 |
|---|---|---|---|
| ~~N1~~ | 계약금 비율 기본값 | ✅ **확정: 20%** (§0.6) | — |
| ~~N3~~ | "착수" 의 정의 | ✅ **확정: 수락 시각** (§0.6) | — |
| **N2** | 중개 수수료율 + **부담 주체** | 5%, 브랜드 부담 | 아래 참고. **P0 프로그램 변경을 막는다** |
| **N4** | 환불 대기 기간 기본값 | 콘텐츠 마감 + 7일 | 타임락 백스톱 값. **P0 프로그램 변경을 막는다** |
