...
| D7 | **플랫폼은 유저 개인키를 보관하지 않는다.** 현재의 커스터디 지갑 자동 생성을 폐기한다 (§P9.1) |
| D8 | **가입 시 지갑은 계속 만들어 준다.** 단 **유저가 소유하는** 논커스터디얼 지갑이다. "web3 몰라도 쓸 수 있다" 는 제품 전제를 포기하지 않는다. **데모 범위는 아니다** (§P9.2) |

D7 과 D8 은 모순이 아니다. **"만들어 준다" 와 "키를 보관한다" 는 별개다** — 이 구분이 §P9.2 의 전부다.
...
| 2 | 지갑이 없다 | **가입 시 유저 소유 지갑을 만들어 준다** (D8, §P9.2) | 플랫폼이 완전한 키도 과반 조각도 갖지 않음 |
| 3 | 2 가 실패하거나 정산 시점까지 미등록 | **정산 보류.** 자금을 에스크로에 남겨 두고, 주소가 등록되면 지급 | 없음 — 보관 주체는 **프로그램 PDA** 다 |

**2번이 기본 경로다**(D8). 3번은 안전망이다.

3번의 발상 전환이 중요하다. 지금은 "주소가 없으면 플랫폼이 **키까지 들고** 만들어 준다" 인데, **"주소가 없으면 기다린다"** 로 바꾸면 커스터디가 사라진다. 에스크로 vault 는 PDA 가 소유하므로 플랫폼이 보관하는 것이 아니다. 그리고 **N4 타임락(마감 + 7일)이 있으니 무한 대기도 아니다** — 기간 내 미등록이면 브랜드에게 환불된다.

즉 커스터디 없이, 자금 유실 없이, 무한 대기 없이 해결된다.

---

## P9.2 — 가입 시 유저 소유 지갑을 만들어 준다 (D8)

### "만들어 준다" ≠ "키를 보관한다"

D7 과 D8 이 모순처럼 보이지만 아니다. 갈리는 지점은 **키가 어디서 생성되고 누가 소유하는가** 하나다.

| | 지금 (폐기 대상) | D8 |
|---|---|---|
| 키 생성 위치 | **서버** (`Keypair()` in `user_wallet.py`) | **유저 디바이스** 또는 임계 조각으로 분산 생성 |
| 비밀키 소유 | **플랫폼** Secret Manager (`knot-user-key-{uid}`) | **유저.** 플랫폼은 완전한 키를 갖지 않는다 |
| 플랫폼 단독 이전 | **가능** ← 이게 커스터디다 | **불가능** |
| 유저 경험 | 구글 로그인만으로 주소 생김 | **동일** |

**유저 경험은 똑같고 소유권과 규제 성격만 바뀐다.** 그래서 제품 전제를 포기할 필요가 없다.

### 구현 방식 후보

| | 방식 | 장점 | 단점 |
|---|---|---|---|
| A | **Passkey 로 감싼 클라이언트 키** — 브라우저에서 keypair 생성, 생체인증에서 유도한 키로 암호화해 **암호문만** 서버 보관 | 진짜 논커스터디얼. 시드문구 없음. 외부 의존 없음 | **기기·passkey 분실 = 영구 손실.** 복구 수단을 플랫폼이 쥐면 커스터디로 회귀. 브라우저 지원 확인 필요 |
| B | **MPC / 임계 서명** (2-of-2, 2-of-3) — 키가 통째로 존재하지 않고 조각으로 생성 | 복구 가능. 플랫폼이 완전한 키를 절대 갖지 않음. 소셜 로그인과 자연스럽게 결합 | 외부 서비스 의존. 조각 배분을 잘못하면 커스터디 해석 |
| C | 스마트 컨트랙트 지갑 | 유연한 권한 설계 | Solana 는 표준 인프라가 약하고, 결국 인출 권한 키를 유저가 가져야 해서 A 나 B 가 또 필요 |

**B 를 기본으로 하고 A 를 보조로 검토한다.** B 는 복구가 가능해서 실사용에 견딜 수 있다. 후보 서비스(Privy, Turnkey, Web3Auth, Dynamic 등)는 **Solana 지원 여부와 조각 배분 구조를 실제로 확인한 뒤** 선정한다.

### 진짜 설계 문제는 복구다

지갑을 만들어 주는 건 쉽다. 어려운 것은 **유저가 기기를 잃었을 때**이고, **복구 설계가 소유권과 규제 해석을 동시에 결정한다.**

2-of-3 구성에서 조각을 이렇게 나눈다고 하자.

```
① 유저 디바이스   ② 플랫폼   ③ 복구용
```

- 플랫폼이 ②와 ③을 **둘 다** 보관하면 2개를 모을 수 있으므로 **사실상 커스터디다.** D7 을 어긴다.
- 따라서 **③을 플랫폼이 가져서는 안 된다.** 유저의 클라우드 백업, 인증 제공자(구글), 또는 독립 제3자에 둔다.

**이 한 줄이 D8 의 성패를 가른다.** 서비스 선정 시 "복구 조각을 우리가 갖는가" 를 첫 질문으로 삼는다.

### 반드시 넣어야 하는 것 — 키 내보내기

외부 서비스에 의존하면 **그 서비스가 사라졌을 때 유저가 자금에 접근할 수 없다.** 그러므로 유저가 원할 때 자기 키를 꺼내 Phantom 등으로 옮길 수 있는 **export 경로가 필수다.**

이것은 소유권의 실질적 증거이자 규제 논거다. **유저가 언제든 키를 통째로 가져갈 수 있다면 그 키는 유저 것이고, 플랫폼이 자산을 지배하지 않는다는 근거가 된다.**

### 실패해도 가입을 막지 않는다

지금 코드도 이미 이 원칙을 지킨다(`user_wallet.py` docstring — "프로비저닝 실패가 가입/역할선택을 막지 않도록 예외는 삼키고 stored=False 로 반환"). 외부 서비스를 쓰면 장애 가능성이 늘어나므로 더 중요해진다.

지갑 생성이 실패하면 **주소 미등록 상태로 두고 §P9.1 의 3번(정산 보류)으로 넘긴다.** 정산 시점까지 등록되면 지급되고, N4 타임락을 넘기면 브랜드에게 환불된다. **자금이 갈 곳 없이 떠 있는 상태가 생기지 않는다.**

### 남는 해석 리스크 — 정직하게

- **해석 리스크가 0인 것은 §P9.1 의 1번(유저가 이미 가진 지갑 연결)뿐이다.**
- D8 은 리스크를 크게 줄이지만 없애지 못한다. 플랫폼이 조각 하나를 갖는 구성이면 "공동 관리 아닌가" 라는 반론 여지가 남는다.
- 완화 근거 3개를 갖춰 둔다: ① **플랫폼 단독 서명 불가** ② **복구 조각 미보유** ③ **상시 export 가능**.
- 실서비스 전환 전 **이 구성을 그대로 들고 법무 확인**을 받는다.

### 데모 범위 — D8 은 데모에 넣지 않는다 (메모, 2026-08-18)

Privy 같은 제공자를 붙이는 것이 정답이지만 **데모 일정 안에서는 불가능하다.** 미루는 것이지 취소가 아니므로 결정 근거를 남긴다.

| | 데모 | 실서비스 |
|---|---|---|
| 0a 서버측 키 생성·보관 제거 | ✅ 넣는다 | ✅ |
| 0b 제공자 기반 유저 소유 지갑 | ❌ **하지 않는다** | ✅ **필수** |
| 지갑 없는 크리에이터 | 데모 시드 계정에 주소를 미리 등록 | 0b 로 해결 |

**데모에서 크리에이터 지갑을 어떻게 하나:** 시드 데모 계정(`c1@knot.com` ~ `c7@knot.com`)에 지갑 주소를 미리 등록해 둔다. 로컬넷 faucet(`web3/gateway/src/faucet.ts`)이 이미 연결된 지갑에 SOL·테스트 USDC 를 채우므로 추가 작업이 적다.

**좋은 소식: 플래그가 이미 기본 off 다.**

`KNOT_USER_WALLET_PROVISION` 의 기본값이 `False` 이고(`config.py:68`), 어떤 `.env*.local` 에도 설정돼 있지 않다(`.env.example` 에만 `=0` 으로 문서화). **즉 현재 서버는 실제로 유저 키를 만들고 있지 않다** — 커스터디 코드는 있지만 꺼진 상태다.

따라서 D7 작업의 실제 크기는 생각보다 작다.

- 플래그를 삭제하지 않고 남긴다. 데모 환경에서만 `true` 로 켠다
- ⚠️ **프로덕션에서 켜면 특금법 노출이 되살아난다.** 배포 체크리스트에 "이 플래그가 false 인지" 항목을 넣는다
- 0a 의 본체는 플래그 제거가 아니라 **서명 챌린지 + 정산 보류**를 만드는 것이다 (지금은 플래그가 꺼져 있으면 크리에이터 주소가 아예 안 잡혀서 정산이 갈 곳이 없다)

플래그로 분리하는 이유: 데모 시연은 "구글 로그인만으로 끝까지" 를 보여줘야 하는데, 지갑 없는 크리에이터를 정산 보류로 두면 **데모가 중간에 끊긴다.** 플래그로 갈라 두면 데모는 매끄럽고 프로덕션 경로는 안전하다.

**0b 착수 시점:** 실서비스 전환 준비를 시작할 때. 그때 제공자 선정의 첫 질문은 "복구 조각을 우리가 갖는가" 다(위 참고).
...
| 파일 | 변경 |
|---|---|
| `backend/apps/api/routes.py:340-360` | 서버측 `provision_user_wallet` 호출 제거. **서버가 키를 만드는 경로를 없앤다** |
...
| 프론트 (신규) | D8 지갑 생성 — 키가 **클라이언트에서** 생성되고 주소만 서버로 등록된다 |
| 정산 경로 | 수령 주소 미등록 시 `SETTLEMENT_PENDING_DESTINATION` 상태로 보류. N4 타임락 경과 시 환불 |
| 크리에이터 온보딩 UI | 지갑 생성(기본) / 기존 지갑 연결(선택) 두 갈래 + **키 내보내기** |
| `docs/13`, `docs/14` | 커스터디 모드 서술 갱신 |

**핵심 원칙 하나로 요약된다: 백엔드에 키를 생성하거나 저장하는 코드가 남아 있으면 안 된다.** 이것을 테스트로 고정한다 — `libs/web3` 에서 `Keypair()` 생성이 사라졌는지, Secret Manager 에 유저 키 경로 쓰기가 없는지.
...
**D7·D8 커스터디 작업은 위 순서와 독립적이며 1·2 와 함께 지금 착수할 수 있다.** 프로그램을 건드리지 않고 백엔드·프론트만 바꾸며, `PLATFORM` 지갑이 devnet 데모 데이터뿐인 **지금이 가장 싸다.**

| 순서 | 범위 | 선행 | 산출물 |
|---|---|---|---|
| 0a | **D7 서버측 키 생성·보관 제거** + 서명 챌린지 + 정산 보류 | 없음 | `.agent/execplans/drop-platform-custody.md` |
| 0b | **D8 유저 소유 지갑 생성** (클라이언트 키 생성 + 키 내보내기) | 제공자 선정 | **데모 범위 아님** — 실서비스 준비 시 착수 (§P9.2 메모) |

0a 와 0b 를 나눈 이유: **0a 는 제공자 선정을 기다리지 않고 지금 할 수 있다.** 서버에서 키를 없애고 "기존 지갑 연결 + 정산 보류" 만으로도 커스터디가 사라진다. 0b 는 그 위에 "지갑 없는 유저도 가입되게" 를 덧붙이는 단계다.
...
|---|---|
| 오프램프 파트너 (P5) | 외부 계약 |
| D8 지갑 제공자 선정 (§P9.2) | Solana 지원 + **복구 조각을 우리가 갖지 않는 구성** 확인 후 결정. **데모에서는 하지 않는다**(§P9.2 메모). §P9.1 의 경로 1·3 만으로도 D7 은 성립하므로 착수를 막지 않는다 |
| 특금법 해석 최종 확인 | 법무. D7·D8 로 노출을 크게 줄였으나 실서비스 전환 전 이 구성 그대로 확인 |
